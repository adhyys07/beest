...
>
	...
	<div class="cg-modal" role="dialog" aria-modal="true" aria-label="Issue card grant" tabindex="-1" onclick={(e) => e.stopPropagation()}>
		...
		{:else if prefill}
			...
			<label class="cg-field">
				<span>Purpose (≤30 chars) <span class="cg-hint">— pre-filled from the item name; any dollar amount is stripped automatically since HCB's pre-auth fraud check reads this text</span></span>
